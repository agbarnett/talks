...
---
## I am not a lone moaner

```{r, out.width='53%'}
knitr::include_graphics(path='figures/van_calster.jpg')
```
...
Over 200 COVID-19 prediction models, but only 1 of any value
...
```{r, fig.height=3.5}
data = read.csv('../youngScientists/data/41586_2022_4800_MOESM4_ESM.csv', skip=2, header=TRUE) %>%
  clean_names() %>%
  filter(year != 'SUM') %>%
...
<img src="https://upload.wikimedia.org/wikipedia/commons/e/ec/Carl_Friedrich_Gauss_1840_by_Jensen.jpg" width="360" height="470"/>
...
```{r, out.width='52%'}
knitr::include_graphics(path='figures/power.png')
```

Actual power from over 1 million RCTs in the Cochrane database
...
---
## Interpretation is not black and white

.pull-left[

"I am a statistician and I know that people find probability and statistics quite difficult to understand, and not intuitive. And after years and years of careful research I have finally concluded that it is because probability and statistics really _are_ difficult to understand and unintuitive."
...
```{r, out.width='80%'}
knitr::include_graphics(path='figures/David_crop.jpg')
```
...
Matching is a very strong assumption, it needs a strong "natural" match (e.g., left and right arm of the same participant)
...
# Statistical robots are coming for you
...
```{r, out.width='72%'}
knitr::include_graphics(path='figures/darren.jpg')
```
